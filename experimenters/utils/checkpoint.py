...
def save_checkpoint(model, optimizer, step: int, path: str, **meta):
    ...
    if optimizer is not None:
        torch.save(optimizer.state_dict(), f"{path}/optim_state.pt")  # Save optimizer state if provided
...
def load_checkpoint(model, optimizer, path: str) -> Dict[str, Any]:
    ...
    model.load_state_dict(torch.load(f"{path}/model_state.pt", map_location="cpu"),
                          strict=False)
    ...
    if optimizer is not None and os.path.exists(f"{path}/optim_state.pt"):
        optimizer.load_state_dict(torch.load(f"{path}/optim_state.pt", map_location="cpu"))
